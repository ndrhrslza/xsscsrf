...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $stmt = $mysqli->prepare("INSERT INTO userlogin (name, matricno, email, curraddress, homeaddress, mobilephone, homephone) VALUES (?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt->execute()) {
            ...
            header("Location: details.html?id=$inserted_id");
    ...
    $stmt = $mysqli->prepare("SELECT * FROM userlogin WHERE id = ?");
